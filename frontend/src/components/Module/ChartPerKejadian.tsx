...
const ChartPerKejadian: React.FC = () => {
  ...
  useEffect(() => {
    const CountKejadianPerJenisKejadian = async () => {
      const response = await api.fetchCountKejadianPerJenisKejadian(
        keyword,
        tahun,
      );
  ...
  const state = [
    {
      name: keyword === "" ? "Semua Bencana" : title,
      data: countKejadian.total,
    },
  ];
  ...
  return (
    <div className="col-span-12 rounded-xl border border-stroke bg-white px-5 pt-7.5 pb-5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-8">
      <div className="mb-4 justify-between gap-4 flex">
        <div>
          <h4 className="text-sm font-semibold text-black dark:text-white lg:text-lg">
            Grafik Kejadian Bencana Berdasarkan Tahun
          </h4>
        </div>
        <div>
          <div className="relative z-20 inline-block">
            <select
              ...
              className="relative z-20 inline-flex appearance-none bg-transparent py-1 pl-3 pr-2 text-sm font-medium outline-none"
            >
...
export default ChartPerKejadian;
