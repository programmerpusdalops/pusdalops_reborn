...
export default function UbahRolePage() {
    ...
    const OnSubmit = async (e: { preventDefault: () => void; }) => {
      e.preventDefault();
      try {
        await api.patchRole(id, role)
        roleAccess.map(async(val) => {
            await api.patchRoleAccess(val?.id_role_access, roleAccess)
        })
        Success()
        navigate("/role")
      } catch (error) {
    ...
    useEffect(() => {
        const RoleAccess = async () => {
            const response = await api.fetchRoleAccessById(id)
                                    ...
                                    <table className="w-full table-auto">
                                        ...
                                        {roleAccess.map((datas, _index) => (
                                                ...
                                                <tr>
                                                    ...
                                                    <input className="relative mr-3" type="hidden" value={datas?.menu} name="menu" onChange={event => handleChangeRole(_index, event)} />
