...
const KontakPentingPage = () => {
  ...
  const OnSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    const formData = new FormData();
      ...
      formData.append("id_jenis_kontak", item?.id_jenis_kontak);
      formData.append("tahun", item?.tahun);
      formData.append("file", file);

    try {
      if (!ids) {
        await api.postKontak(formData)
        Success()
        Reload()
      } else {
        await api.patchKontak(ids, formData)
        Success()
        Reload()
      }
    } catch (error) {
  ...
  const Clear = () => {
    setItem({...item, judul: "", tahun: "", id_jenis_: ""})
    setIds(null)
  }
  ...
  return (
    <>
      <Breadcrumb pageName="kontak Penting Page" />
