...
export default function UbahDataBencanaPage() {
  ...
  const OnSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    for (let i = 0; i < data.length; i++) {
      if (!data[i].kab || !data[i].kec || !data[i].desa) {
        Swal.fire({
          icon: 'warning',
          title: 'Data Lokasi Belum Lengkap',
          text: `Kabupaten, Kecamatan, dan Desa/Kelurahan pada terdampak ke-${i + 1} wajib diisi.`,
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        await api.patchKorban(data[i]?.id_korban, korban[i])
        await api.patchKerusakan(data[i]?.id_kerusakan, kerusakan[i])
        await api.patchLokasi(data[i]?.id_lokasi, lokasi[i])
      }
      Success()
    } catch (error) {
      Error()
                ...
                <Fragment key={_index}>
                  ...
                  <div className="mb-4.5 flex flex-col gap-6 xl:flex-row py-3">
                    <div className="w-full">
                      <label className="mb-2.5 block text-black dark:text-white">
                        Kabupaten <span className="text-meta-1">*</span>
                      </label>
                    ...
                    <div className="w-full">
                      <label className="mb-2.5 block text-black dark:text-white">
                        Desa/Kelurahan <span className="text-meta-1">*</span>
                      </label>
