...
const JenisDokumenPage = () => {
  ...
  const OnSubmit = async (e: { preventDefault: () => void; }) => {
      ...
      try {
        if(!ids){
          await api.postJenisDokumen(item)
          setItem({...item, id: uuidv4(), nama: ""})
          Success()
          Reload()
        }else{
          await api.patchJenisDokumen(ids, item)
          Success()
          Reload()
        }
      } catch (error) {
  ...
  const OnDelete = async (id: any) => {
    try {
      await api.deleteJenisDokumen(id)
      Success()
      Reload()
    } catch (error) {
      Error()
  ...
  useEffect(() => {
    if(ids){
      const Reload = async () => {
        const response = await api.fetchJenisDokumenById(ids);
  ...
  return (
    <>
      <Breadcrumb pageName="Jenis Dokumen" />
